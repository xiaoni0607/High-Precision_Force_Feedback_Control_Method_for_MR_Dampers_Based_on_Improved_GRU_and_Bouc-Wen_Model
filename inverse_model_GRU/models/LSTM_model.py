...
def persistence_baseline(last_phys: np.ndarray, horizon: int) -> np.ndarray:
    """生成持久化基线（即重复最后一个值）"""
    return np.repeat(last_phys, horizon, axis=1)

@torch.no_grad()
def debug_test_stats(loader, model):
    """调试函数，用于评估模型在测试集上的MSE和标准差"""
    model.eval()
    mse_sum, n = 0.0, 0
    for xb, _, yb in loader:
        pr, _ = model(xb)
        mse_sum += torch.mean((pr - yb) ** 2).item()
        n += 1
    mse_avg = mse_sum / max(1, n)
    print(f"[TEST] 模型MSE: {mse_avg:.4f}")

    # 可选：计算并打印预测值和目标值的标准差
    all_preds, all_trues = [], []
    for xb, _, yb in loader:
        pr, _ = model(xb)
        all_preds.append(pr.detach().cpu().numpy())  # 使用 .detach()
        all_trues.append(yb.detach().cpu().numpy())  # 同样使用 .detach()
    all_preds = np.concatenate(all_preds, axis=0)
    all_trues = np.concatenate(all_trues, axis=0)
    pred_std = np.std(all_preds)
    true_std = np.std(all_trues)
    print(f"[TEST] 预测标准差: {pred_std:.3f}, 目标标准差: {true_std:.3f}")
    ...
    def _build_and_split(self):
        ...
        def build(idxs):
            ...
            if self.include_future_exog and len(self.exog_idx) > 0:
                ...
                rep = Xf_s.shape[1]
                feats.extend([                     np.repeat(Xn_exog_s[:, None, :], rep, axis=1),      # [N,L,E]
...
class LSTMHead(nn.Module):
    ...
    def forward(self, x):
        ...
        last_inp = x[:, -1, :]
        out = self.head(torch.cat([lstm_out[:, -1, :], last_inp], dim=-1))  # [B,H]
        return out, None  # 返回预测结果，第二个为 None
...
def main():
    ...
    # === 改成你的 CSV 路径 ===
    csv_path = "D:/desktop/杂/个人有关/毕业论文/倪浩君大论文/force_control_dataset.csv"
    ...
    lag, corr = estimate_delay(true_track, pred_track, max_lag=500)
    ...
    # 作图（含 step1_only / avg_track）
    plot_all(tr_loss, va_loss, trues_phys, preds_phys, baseline_phys, outdir="figs_LSTM_MRB", interval=100)
