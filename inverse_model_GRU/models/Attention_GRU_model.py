# -*- coding: utf-8 -*-
"""
MR Damper 多步预测（H步）完整脚本（集成：事件采样 + 事件加权MSE + 诊断 + 可视化修正 + 信号时延估计-亚采样精度）
...
- 新增: 信号时延估计（去重叠平均 + 归一化互相关 + 抛物线插值到亚采样精度）；仅打印结果，不新增图像
"""
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, g_out):
        w = torch.softmax(self.attn(g_out).squeeze(-1), dim=1)  # [B,L]
        ctx = torch.sum(g_out * w.unsqueeze(-1), dim=1)         # [B,H]
        return ctx, w

class GRUAttnHead(nn.Module):
    """GRU + Attention + 'last-step shortcut' head（ctx ⊕ last_inp → MLP → H维输出）"""
    ...
    def forward(self, x):
        ...
        ctx, w   = self.attn(g_out)
        last_inp = x[:, -1, :]
        out      = self.head(torch.cat([ctx, last_inp], dim=-1))  # [B,H]
        return out, w
...
def print_metrics(true_values, predicted_values, true_test_values, predicted_test_values):
    from sklearn.metrics import mean_squared_error, mean_absolute_error, r2_score

    mse = mean_squared_error(true_values, predicted_values)
    mae = mean_absolute_error(true_values, predicted_values)
    rmse = np.sqrt(mse)
    r2 = r2_score(true_test_values, predicted_test_values)
...
# ---------------- Delay Estimation (Signal, sub-sample) ----------------
...
# ---------------- Baseline / Plot ----------------
def persistence_baseline(last_y_raw, H): return np.repeat(last_y_raw, H, axis=1)

def plot_all(tr_loss, va_loss, trues, preds, base, outdir, interval=500):
    """
    修改后的 plot_all 函数，分别生成训练集和测试集的预测结果图
    """
    ...
    plt.plot(va_loss, label="val_loss")
    plt.title("Training & Validation Loss (z-MSE)")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.tight_layout()
    ...
    time_steps = np.arange(0, len(trues))  # 时间步
    interval_idx = np.arange(0, len(trues), interval)  # 间隔步长

    # 在训练集和测试集都使用相同的 interval 进行采样
    plt.plot(time_steps[interval_idx], trues[interval_idx], linestyle="--", linewidth=2, label="Actual")
    plt.plot(time_steps[interval_idx], preds[interval_idx], linewidth=1, label="Predicted")
    plt.title("Prediction vs True Values")
    plt.xlabel("Time Steps")
    plt.ylabel("Measured Current (A)")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(outdir, "pred_vs_true.png"), dpi=150)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(outdir, "residuals.png"), dpi=150)
...
def main():
    ...
    preds_train_phys, trues_train_phys = trainer.predict_phys(train_loader, to_phys)
    preds_test_phys, trues_test_phys = trainer.predict_phys(test_loader, to_phys)

    assert preds_train_phys.shape == trues_train_phys.shape, f"shape mismatch: {preds_train_phys.shape} vs {trues_train_phys.shape}"
    assert preds_test_phys.shape == trues_test_phys.shape, f"shape mismatch: {preds_test_phys.shape} vs {trues_test_phys.shape}"

    # 基线（物理单位；持久化基线：把 y_t 持久化为 y_{t+1..t+H}）
    baseline_train_phys = persistence_baseline(data.last_train.cpu().numpy(), horizon)
    baseline_test_phys = persistence_baseline(data.last_test.cpu().numpy(), horizon)

    # 打印训练集和测试集的性能指标
    print_metrics(trues_train_phys, preds_train_phys, trues_test_phys, preds_test_phys)

    # 为训练集和测试集分别保存图像到不同文件夹
    plot_all(tr_loss, va_loss, trues_train_phys, preds_train_phys, baseline_train_phys, outdir="fig_GRU_Attention_MRB/train_results", interval=500)
    plot_all(tr_loss, va_loss, trues_test_phys, preds_test_phys, baseline_test_phys, outdir="fig_GRU_Attention_MRB/test_results", interval=500)
